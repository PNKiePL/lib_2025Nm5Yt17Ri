...
public:
	...
	int retRgbTO256色_大于(int r, int g, int b){ //rgb-->i,颜色转换
		int 颜色 = -1;
		if( r>=255 ){
			if( g>=0 ){
				if( b>=0 ){ retu Red; }
	...
	/*算法选哪一个: {0=小于递增,1=大于地减,}*/
	int retRgbTO256色(int r, int g, int b, int 算法){
		if( 算法==0 ){
			retu retRgbTO256色_小于(r, g, b);
		}/*if*/
		if( 算法==1 ){
			retu retRgbTO256色_大于(r, g, b);
		}/*if*/
		retu-1;
	}
	...
	/*复制数据,Mat-->v2i; 厨法应该=小于*/
	template<typename at类型>
	void get图片TO256色(/*输入*/Mat& 图, string 大于or小于,/*输出*/新v2i& v2i256色){
		get图片TO256色的assign(图, v2i256色);
		if( 大于or小于=="大于" ){
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号++ ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号++ ){
					v2i256色[y序号][x序号] = retRgbTO256色_大于(图.at<at类型>(Point(x序号, y序号))[2], 图.at<at类型>(Point(x序号, y序号))[1], 图.at<at类型>(Point(x序号, y序号))[0]);
				}/*for*/
			}/*for*/
			retu;
		}/*if*/
		if( 大于or小于=="小于" ){
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号++ ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号++ ){
					v2i256色[y序号][x序号] = retRgbTO256色_小于(图.at<at类型>(Point(x序号, y序号))[2], 图.at<at类型>(Point(x序号, y序号))[1], 图.at<at类型>(Point(x序号, y序号))[0]);
				}/*for*/
			}/*for*/
		}/*if*/
	}
	...
	template<typename at类型>
	void get图片TO256色(/*输入*/Mat& 图, int y下个点是加几个像素, int x下个点是加几个像素, string 大于or小于,/*输出*/新v2i& v2i256色){
		...
		if( 大于or小于=="大于" ){
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号 += y下个点是加几个像素 ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号 += x下个点是加几个像素 ){
					...
					//	cmd1.清屏(); c了几行 = 0;
					//}
					v2i256色[输出图片的坐标.Y][输出图片的坐标.X] = retRgbTO256色_大于(图.at<at类型>(Point(x序号, y序号))[2], 图.at<at类型>(Point(x序号, y序号))[1], 图.at<at类型>(Point(x序号, y序号))[0]);
					输出图片的坐标.X++;
				}/*for*/
				输出图片的坐标.X = 0;
				输出图片的坐标.Y++;
		...
		if( 大于or小于=="小于" ){
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号 += y下个点是加几个像素 ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号 += x下个点是加几个像素 ){
					v2i256色[输出图片的坐标.Y][输出图片的坐标.X] = retRgbTO256色_小于(图.at<at类型>(Point(x序号, y序号))[2], 图.at<at类型>(Point(x序号, y序号))[1], 图.at<at类型>(Point(x序号, y序号))[0]);
					输出图片的坐标.X++;
				}/*for*/
				输出图片的坐标.X = 0;
				输出图片的坐标.Y++;
	...
	template<typename at类型>
	void get图片TO256色atyx(/*输入*/Mat& 图, int y下个点是加几个像素, int x下个点是加几个像素, string 大于or小于,/*输出*/新v2i& v2i256色){
		get图片TO256色的assign(图, y下个点是加几个像素, x下个点是加几个像素, v2i256色);
		if( 大于or小于=="大于" ){
			v2i256色.assign((图片尺寸.Y%y下个点是加几个像素>0) ? (图片尺寸.Y/y下个点是加几个像素+1) : (图片尺寸.Y/y下个点是加几个像素), 新vi((图片尺寸.X%x下个点是加几个像素>0) ? (图片尺寸.X/x下个点是加几个像素)+1 : (图片尺寸.X/x下个点是加几个像素), 0));
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号 += y下个点是加几个像素 ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号 += x下个点是加几个像素 ){
					v2i256色[输出图片的坐标.Y][输出图片的坐标.X] = retRgbTO256色_大于(图.at<at类型>(Point(y序号, x序号))[2], 图.at<at类型>(Point(y序号, x序号))[1], 图.at<at类型>(Point(y序号, x序号))[0]);
					输出图片的坐标.X++;
				}/*for*/
				输出图片的坐标.X = 0;
				输出图片的坐标.Y++;
		...
		if( 大于or小于=="小于" ){
			for( size_t y序号 = 0; y序号<图片尺寸.Y; y序号 += y下个点是加几个像素 ){/*行*/
				for( size_t x序号 = 0; x序号<图片尺寸.X; x序号 += x下个点是加几个像素 ){
					v2i256色[输出图片的坐标.Y][输出图片的坐标.X] = retRgbTO256色_小于(图.at<at类型>(Point(y序号, x序号))[2], 图.at<at类型>(Point(y序号, x序号))[1], 图.at<at类型>(Point(y序号, x序号))[0]);
					输出图片的坐标.X++;
				}/*for*/
				输出图片的坐标.X = 0;
				输出图片的坐标.Y++;
	...
	auto cRgb(Mat& m, int y, int x){
		...
		coutsf3("第{}行`第{}列={}", y, x, ret名称_256色iTOstr(retRgbTO256色(r, g, b, 1)));
		couta1(" {");
		couta2("r=", r);
		couta2("g=", g);
		couta3("b=", b, "}\n");
	...
	void 例子_截图转换v2i(新v2i &v2i256图片){
		int 图片类型; int 图片类型2,除数=1;
		Mat M,M2;
		jxtu.get截图(M, 图片类型/*, 10000*/);
		get图片TO256色<Vec4b>(M,"小于",v2i256图片);
		getV2i256色图TOMat图片<Vec4b>(v2i256图片,M2);
	}/*void*/
	void 例子_for所有像素(新v2i &v2i256图片){
		下标info 所y像s;
		for( 所y像s.一/*y*/ = 0; 所y像s.一 < 0; 所y像s.一++ ){
			for( 所y像s.二/*x*/ = 0; 所y像s.二 < 0; 所y像s.二++ ){
				v2i256图片[ 所y像s.一][ 所y像s.二];
			}/*for*/
		}/*for*/
	}/*void*/
};
